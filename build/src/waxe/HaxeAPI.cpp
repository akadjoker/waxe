...
float Val2Float(value inVal, float inDefault)
{
   return ValueToFloat(inVal);
}
...
value WXToValue(const float &inVal)
{
return alloc_float(inVal);
}
...
value wx_byte_array_init(value inFactory, value inLen, value inResize, value inBytes)
{
...
DEFINE_PRIM(wx_byte_array_init,4);


value wx_set_resource_factory(value inFactory)
{
   gResourceFactory = new AutoGCRoot(inFactory);
   return alloc_null();
}
DEFINE_PRIM(wx_set_resource_factory,1);

value wx_byte_array_read_file(value inFilename)
{
   ByteArray result = ByteArray::FromFile(Val2Str(inFilename));
   return result.mValue;
}
DEFINE_PRIM(wx_byte_array_read_file,1);


value wx_byte_array_get_native_pointer(value inByteArray)
{
...
DEFINE_PRIM(wx_byte_array_get_native_pointer,1);
